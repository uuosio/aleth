...
json_spirit::mObject fillBCTest(json_spirit::mObject const& _input, bool _allowInvalidBlocks)
{
    ...
    if (testChain.topBlock().state().addresses().size() <= 20 || Options::get().fullstate)
...
void testBCTest(json_spirit::mObject const& _o)
{
    ...
    else
    {
        ...
        string postHash = _o.at("postState").get_str();
        string message = testName + "Final StateRoot is different! (Look at expect section in the test /src Filler file, or run the test generation with --fullstate flag to geth the full post state.";
        BOOST_CHECK_MESSAGE(postHash == toHexPrefixed(testChain.topBlock().state().rootHash()), message);
        BOOST_CHECK_MESSAGE(postHash == toHexPrefixed(blockchain.topBlock().state().rootHash()), message);
    }
}
